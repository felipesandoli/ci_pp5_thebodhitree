...
- Feedback messages to be added for user interactions to the website, such as loging in and out or confirming, editing and deleting a booking.
...
The following manual testings were performed according to user stories acceptance criteria:

- Home page has navigation link that is responsive and works across multiple devices.
- Home page displays list of featured properties.
- The property details page contains all relevant information about the property.
- The property details page has a functioning button that allows the user to easily book a stay at that property.
- Stripe payments works as expected except when the payment amount is large enough which results in an error.
- Submitting the booking form creates the booking on database.
- Booking form is only accepted upon entering valid data.
- My bookings page is only accessible for authenticated users.
- Clicking on a booking on my booking list successfully redirects the user to the booking details page where they can update the booking details or cancel it.
- Retrieve my booking is only accessible for users that are not authenticated.
- Retrieve my booking form only works with valid booking reference number.
- Accessing the booking through retrieve my booking allows the user to edit or cancel the booking.
- Email confirmation is sent upon registering a new account.
